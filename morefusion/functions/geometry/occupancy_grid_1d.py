...
class OccupancyGrid1D(chainer.Function):
    ...
    def check_type_forward(self, in_types):
        ...
        chainer.utils.type_check.expect(
            points_type.dtype == np.float32, points_type.ndim == 1,
        )
    ...
    def forward(self, inputs):
        ...
        I, J = xp.meshgrid(xp.arange(dimension), xp.arange(points.shape[0]),)
...
def occupancy_grid_1d(points, *, pitch, origin, dimension):
    ...
    d_IJ = OccupancyGrid1D(pitch=pitch, origin=origin, dimension=dimension)(
        points
    )
    ...
    def check_backward(points_data, grad_m):
        chainer.gradient_check.check_backward(
            lambda x: occupancy_grid_1d(
                x, pitch=pitch, origin=origin, dimension=dimension,
            ),
            points_data,
            grad_m,
        )
