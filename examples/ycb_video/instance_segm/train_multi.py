...
class TrainChain(chainer.Chain):
    ...
    def forward(self, imgs, bboxes, labels, masks=None):
        ...
        pad_size = (
            np.ceil(pad_size / self.model.stride) * self.model.stride
        ).astype(int)
        x = np.zeros(
        ...
        head_loc_loss, head_conf_loss = bbox_head_loss_post(
            head_locs, head_confs, roi_indices, head_gt_locs, head_gt_labels, B
        )
...
def transform_dataset(dataset, model, train):
    if train:
        transform = Compose(
            ...
            AsType(
                ["rgb", "labels", "bboxes"], [np.float32, np.int32, np.float32]
            ),
            HWC2CHW(["rgb"]),
...
def main():
    ...
    model = MaskRCNNFPNResNet50(
        n_fg_class=len(fg_class_names), pretrained_model="imagenet"
    )
