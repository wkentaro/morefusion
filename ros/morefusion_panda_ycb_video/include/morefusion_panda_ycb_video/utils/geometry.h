...
void track_instance_id(
  ...
  std::set<int> instance_ids1 = morefusion_panda_ycb_video::utils::unique<int>(reference);
  std::set<int> instance_ids2 = morefusion_panda_ycb_video::utils::unique<int>(*target);
  ...
  for (int ins_id2 : instance_ids2) {
    ...
    if (morefusion_panda_ycb_video::utils::is_detected_mask_too_small(mask2)) {
  ...
  instance_ids1 = morefusion_panda_ycb_video::utils::unique<int>(*target);
  cv::Mat merged(reference.size(), CV_32SC1, -2);
  for (int ins_id2 : morefusion_panda_ycb_video::utils::unique<int>(reference)) {
    if (ins_id2 < 0) {
      continue;
    }
