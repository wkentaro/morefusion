...
class ObjectPoseInterface:
    def __init__(self, object_models):
        ...
        (
            ...
            canonical_extents,
        ) = self._get_cannonical_object_rotation_mats_and_extent_idxs()
        self._get_class_specific_orientations_and_extents(
            canonical_quaternions, canonical_extents
        )
    ...
    def _get_remaining_axis_key(axis_1, axis_2):
        if (axis_1[0] == "y" and axis_2[0] == "z") or (
            axis_2[0] == "y" and axis_1[0] == "z"
        ):
            return "x+"
        elif (axis_1[0] == "x" and axis_2[0] == "z") or (
            axis_2[0] == "x" and axis_1[0] == "z"
        ):
            return "y+"
        elif (axis_1[0] == "x" and axis_2[0] == "y") or (
            axis_2[0] == "x" and axis_1[0] == "y"
        ):
            return "z+"
    ...
    def _get_class_specific_orientations_and_extents(
        self, cannonincal_Rs, cannonical_extent_idxs
    ):
        ...
        for class_id in self._class_ids:
            ...
            R_z_fix = ttf.rotation_matrix(
                self._z_rotations[class_id], [0.0, 0.0, 1.0]
            )
    ...
    def _get_object_poses(self, class_id, position_on_table):
        ...
        object_z_positions = [
            self._floor_z + extent[2] / 2 for extent in object_extents
        ]
        object_positions = [
            list(position_on_table) + [z_position]
            for z_position in object_z_positions
        ]
        return [
            list(object_position) + list(object_quaternion)
            for object_position, object_quaternion in zip(
                object_positions, object_quaternions
            )
        ]

    def get_robot_poses(
        self, class_id, object_to_robot_mat, position_on_table
    ):
        object_poses = self._get_object_poses(class_id, position_on_table)
        object_mats = [
            gk.quaternion_pose_to_mat(np, pose) for pose in object_poses
        ]
        robot_mats = [
            np.matmul(object_mat, object_to_robot_mat)
            for object_mat in object_mats
        ]
        robot_poses = [
            gk.mat_to_quaternion_pose(np, robot_mat)
            for robot_mat in robot_mats
        ]
