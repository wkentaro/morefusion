...
def mat_to_relative_mat(
    f, cam_inv_extrinsic_mat, cam_extrinsic_mat, batch_size
):

    cam_inv_extrinsic_mat_full = f.concatenate(
        (
            cam_inv_extrinsic_mat,
            f.reshape(
                f.tile(f.array([0.0, 0.0, 0.0, 1.0]), [batch_size]),
                (batch_size, 1, 4),
            ),
        ),
        -2,
...
def mat_to_relative_pose(
    f, cam_inv_extrinsic_mat, cam_extrinsic_mat, batch_size
):

    relative_mat = mat_to_relative_mat(
        f, cam_inv_extrinsic_mat, cam_extrinsic_mat, batch_size
    )
...
def mat_to_relative_quaternion_pose(
    f, cam_inv_extrinsic_mat, cam_extrinsic_mat, batch_size
):
...
def increment_quaternion_pose_with_velocity(f, pose, velocity, control_dt):
    ...
    quaternion_transform = scale_quaternion_theta(
        f, quaternion_vel, control_dt
    )
    new_quaternion = hamilton_product(
        f, current_quaternion, quaternion_transform
    )
    new_pose = f.concatenate(
        (f, pose[0:3] + velocity[0:3] * control_dt, new_quaternion)
    )
...
def rot_mat_to_euler(f, rot_mat):
    ...
    c1 = _math.cos(alpha)
    gamma = _math.atan2(
        s1 * rot_mat[2, 0] - c1 * rot_mat[1, 0],
        c1 * rot_mat[1, 1] - s1 * rot_mat[2, 1],
    )
...
def euler_to_rot_mat(f, euler_angles):
    ...
    rot_y = f.array(
        [
            [_math.cos(beta), 0, _math.sin(beta)],
            [0, 1, 0],
    ...
    rot_z = f.array(
        [
            [_math.cos(gamma), -_math.sin(gamma), 0],
