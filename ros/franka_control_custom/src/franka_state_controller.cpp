...
#include <franka_control_custom/franka_state_controller.h>
...
namespace franka_control_custom {
    ...
    bool FrankaStateController::init(hardware_interface::RobotHW* robot_hardware,
        ...
        ef_pose_publisher = controller_node_handle.advertise<std_msgs::Float64MultiArray>("/end_effector_pose", 1);
        joint_angles_publisher = controller_node_handle.advertise<std_msgs::Float64MultiArray>("/joint_angles", 1);
    ...
    void FrankaStateController::update(const ros::Time& time, const ros::Duration& /* period */) {
        ...
        // publish end effector pose at 1000 hz
        std::array<double,16> ef_matrix = robot_state_.O_T_EE;
        std_msgs::Float64MultiArray ef_pose_msg;
        ef_pose_msg.data.clear();
        ef_pose_msg.data.push_back(time.toSec());
        for (int i=0; i<16; i++)
        {
            ef_pose_msg.data.push_back(ef_matrix[i]);
        }
        ef_pose_publisher.publish(ef_pose_msg);
        
        // publish joint angles at 1000 hz
        std::array<double,7> joint_angles = robot_state_.q;
        std_msgs::Float64MultiArray joint_angles_msg;
        joint_angles_msg.data.clear();
        joint_angles_msg.data.push_back(time.toSec());
        for (int i=0; i<7; i++)
        {
            joint_angles_msg.data.push_back(joint_angles[i]);
        }
...
PLUGINLIB_EXPORT_CLASS(franka_control_custom::FrankaStateController, controller_interface::ControllerBase)