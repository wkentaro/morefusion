...
#include "morefusion_panda_ycb_video/OctomapServer.h"
...
OctomapServer::OctomapServer() {
  ...
  pub_grids_ = pnh_.advertise<morefusion_panda_ycb_video::VoxelGridArray>("output/grids", 1);
  pub_grids_noentry_ = pnh_.advertise<morefusion_panda_ycb_video::VoxelGridArray>(
  ...
  pub_class_ = pnh_.advertise<morefusion_panda_ycb_video::ObjectClassArray>("output/class", 1);
  ...
  sub_class_ = new message_filters::Subscriber<morefusion_panda_ycb_video::ObjectClassArray>(
    pnh_, "input/class", 5);
  ...
  client_render_ = pnh_.serviceClient<morefusion_panda_ycb_video::RenderVoxelGridArray>("render");

  dynamic_reconfigure::Server<morefusion_panda_ycb_video::OctomapServerConfig>::CallbackType f =
...
void OctomapServer::configCallback(
  const morefusion_panda_ycb_video::OctomapServerConfig& config, const uint32_t level) {
...
void OctomapServer::insertCloudCallback(
    ...
    const morefusion_panda_ycb_video::ObjectClassArrayConstPtr& class_msg) {
  // Get TF
  ...
  morefusion_panda_ycb_video::utils::track_instance_id(
  ...
  morefusion_panda_ycb_video::ObjectClassArray cls_rend_msg;
  ...
  for (std::map<int, unsigned>::iterator it = class_ids_.begin();
    ...
    morefusion_panda_ycb_video::ObjectClass cls;
  ...
  std::set<int> instance_ids_active = morefusion_panda_ycb_video::utils::unique<int>(label_ins_rend);
...
void OctomapServer::insertScan(
  ...
  for (int instance_id : instance_ids) {
    ...
    if (instance_id >= 0) {
      ...
      pitch = morefusion_panda_ycb_video::utils::class_id_to_voxel_pitch(class_id);
    }
...
void OctomapServer::getGridsInWorldFrame(
    ...
    morefusion_panda_ycb_video::VoxelGridArray& grids) {
  ...
  for (std::map<int, OcTreeT*>::iterator it_octree = octrees_.begin();
    ...
    double pitch = morefusion_panda_ycb_video::utils::class_id_to_voxel_pitch(class_id);

    // world frame
    octomap::point3d center = centers_.find(instance_id)->second;

    morefusion_panda_ycb_video::VoxelGrid grid;
    grid.pitch = pitch;
    grid.dims.x = 32;
    grid.dims.y = 32;
    grid.dims.z = 32;
...
void OctomapServer::publishGrids(
  ...
  grids.header.stamp = rostime;
  morefusion_panda_ycb_video::VoxelGridArray grids_noentry;
  ...
  for (std::map<int, OcTreeT*>::iterator it_octree = octrees_.begin();
    ...
    morefusion_panda_ycb_video::VoxelGrid grid_noentry;
    ...
    if (publishMarkerArray) {
      for (unsigned i= 0; i < occupiedNodesVis.markers.size(); ++i) {
        ...
        occupiedNodesVis.markers[i].color =
          morefusion_panda_ycb_video::utils::colorCategory40(instance_id + 1);
...
int main(int argc, char** argv) {
  ...
  morefusion_panda_ycb_video::OctomapServer server;
  ros::spin();
  return 0;
}
