...
from morefusion_ros.srv import MoveToJointPosition
from morefusion_ros.srv import SetSuction
...
class RobotInterface:
    ...
    def _set_suction_on(self, on):
        ...
        if not response.success:
            raise RuntimeError(f"Failed to set suction on: {on}")
    ...
    def move_to_overlook_pose(self, scale_vel=None, scale_accel=None):
        return self._set_joint_position(
            self._joint_positions["overlook"], scale_vel, scale_accel,
        )

    def move_to_reset_pose(self, scale_vel=None, scale_accel=None):
        return self._set_joint_position(
            self._joint_positions["reset"], scale_vel, scale_accel,
        )
