...
class RobotDemoInterface(RobotInterface):
    def __init__(self):
        ...
        self._object_filepaths = [
            self._object_models.get_cad_file(i)
            for i in range(1, len(self._object_models.class_names))
        ]
        self._collision_filepaths = [
            morefusion.utils.get_collision_file(cad_filename)
            for cad_filename in self._object_filepaths
        ]
        self._collision_meshes_multi = [
            trimesh.load_mesh(str(filepath), process=False)
            for filepath in self._collision_filepaths
        ]
    ...
    def _broadcast_object_pose(self):
        translation = self._pick_point_np_poses_in_world_frame[
            self._object_id_to_grasp
        ][0:3]
        rotation = self._pick_point_np_poses_in_world_frame[
            self._object_id_to_grasp
        ][3:]
    ...
    def _get_grasp_pose(self):
        ...
        grasp_mat_in_obj_frame = np.array(
            [
                [1, 0, 0, 0],
                [0, 1, 0, 0],
        ...
        grasp_mat_in_world_frame = np.matmul(
            self._pick_point_mats_in_world_frame[self._object_id_to_grasp],
            grasp_mat_in_obj_frame,
        )
    ...
    def _filter_robot_poses(self, robot_poses):
        ...
        for robot_pose in robot_poses:
            ...
            R = ttf.quaternion_matrix(np.array([ori.w, ori.x, ori.y, ori.z]))[
                0:3, 0:3
            ]
            z_vector = np.array([0.0, 0.0, 1.0]).transpose().reshape((3, 1))
            down_z_vector = (
                np.array([0.0, 0.0, -1.0]).transpose().reshape((3, 1))
            )
    ...
    def _check_if_all_distractors_removed(self):
        if (
            len(self._picked_objects)
            < len(self._ordered_object_ids_to_grasp) - 1
        ):
            return False
        return True
    ...
    def _pick_poses_callback(self, pick_point_poses):
        ...
        for object_pose in pick_point_poses.poses:
            ...
            self._pick_point_np_poses_in_world_frame[
                object_pose.class_id
            ] = pick_point_np_pose_in_world_frame
            ...
            self._pick_point_mats_in_world_frame[
                object_pose.class_id
            ] = pick_point_mat_in_world_frame

            self._class_id_to_instance_id[
                object_pose.class_id
            ] = object_pose.instance_id
    ...
    def _object_poses_callback(self, object_poses):
        for object_pose in object_poses.poses:
            ...
            self._object_mats_in_world_frame[
                object_pose.class_id
            ] = object_mat_in_world_frame

            self._object_pose_msgs_in_world_frame[
                object_pose.class_id
            ] = object_pose_in_world_frame
    ...
    def _update_static_scene(self):
        ...
        for (
            class_id,
            object_pose,
        ) in self._object_pose_msgs_in_world_frame.items():
    ...
    def _update_scene_with_grasp(self):
        ...
        mesh_pose = self._object_pose_msgs_in_world_frame[
            self._object_id_to_grasp
        ]
        self._world_interface.add_attached_meshes(
            [str(self._object_id_to_grasp)],
            [mesh_to_grasp],
            [mesh_pose],
            ["panda_suction_cup"],
        )
    ...
    def _update_scene_with_placement(self, pose):
        ...
        self._world_interface.add_attached_meshes(
            [str(self._object_id_to_grasp)],
            [self._collision_meshes[self._object_id_to_grasp - 1]],
            [pose],
            ["panda_link0"],
        )
